...
path = "/work/ef17148/acrc_footprints/data_generation/wind_to_fp_simple/"
...
for month in filenames:
    ...
    for mode in ["train", "test", "val"]:
        ...
        timestamps = random.sample(range(0, len(met_data.time.values)), nsamples[mode])
        ...
        for s in range(nsamples[mode]):
            ...
            ax.contourf(LON, LAT, np.log10(fp),level=3)
            ...
            if "Pressure" in variables:
                cs = ax.contour(LON, LAT, met_data_cut["Pressure"],levels=10)
            ax.scatter(fp_data.release_lon[0], fp_data.release_lat[0], color="red")
            ax.axis('off')
